...
def write_per_op(d):
    for op in OPS:
        ...
        plot_op(d, op, ax)
        ax.set_title(f"{TITLES[op]}  —  {TAG}", fontweight="bold", fontsize=15, pad=12)
        fig.tight_layout()
...
def write_grid(d):
    ...
    fig.suptitle(f"test1_16M: Thread Scalability (V1 vs V2)  —  {TAG}",
...
def write_speedup(d):
    ...
    x = np.arange(len(ops))
    ratio = [d["YFastV2"][o][64] / d["YFastV1"][o][64] for o in ops]
    fig, ax = plt.subplots(figsize=(11, 6))
    bars = ax.bar(x, ratio, 0.55, color="#34d399", edgecolor="#1e293b",
                  label="V2 / V1 at 64 threads")
    ax.axhline(1.0, color="#fbbf24", ls="--", lw=1.4, label="V1 baseline")
    ax.set_xticks(x)
    ax.set_xticklabels([o.capitalize() for o in ops])
    ax.set_ylabel("Speedup (V2 / V1)")
    ax.set_title(f"Y-Fast V2 speedup over V1 at 64 threads  —  {TAG}",
    ...
    for bar in bars:
        ...
        ax.text(bar.get_x() + bar.get_width()/2, h + 0.05, f"{h:.2f}x",
                ha="center", fontsize=10, fontweight="bold", color="#34d399")
    fig.tight_layout()
    fig.savefig(os.path.join(HERE, "speedup_v2_over_v1_64t.png"))
    plt.close(fig)
    print("  speedup_v2_over_v1_64t.png")
